...
def test(model, visual_model, testdataloader, maxlen, prompt_text, gt, gtsegments, gtlabels, device):    
    ...
    with torch.no_grad():
        for i, item in enumerate(testdataloader):
            ...
            #sys.stdout.write(f'prob_av: {prob_av.shape}\n')
            #sys.stdout.write(f'prob2: {prob2.shape}\n')
            #sys.stdout.flush()
            
            element_logits2 = logits2[0:len_cur].softmax(dim=-1).detach().cpu().numpy() # [1 * 256, 7]
...
if __name__ == '__main__':
    ...
    model_param = torch.load(args.checkpoint_path)
