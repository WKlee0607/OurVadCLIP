...
def CLAS2(logits, labels, lengths, device):
    ...
    return clsloss
"""
def normalize_logits(x, clip_val=10.0):
    x = x - x.mean(dim=1, keepdim=True)  # 중심화
    x = x / (x.std(dim=1, keepdim=True) + 1e-6)  # 표준편차 정규화
...
def distill_cosine_fixed_teacher(teacher_logits, student_logits, T=3.0):
    with torch.no_grad():# detach teacher to stop gradient
        teacher_probs = F.softmax(teacher_logits / T, dim=1)
    student_probs = F.softmax(student_logits / T, dim=1)
    loss = 1 - F.cosine_similarity(student_probs, teacher_probs, dim=1).mean()
    return loss
...
def AP2_DISTILL(logits_target, logits_source, temperature):
    ...
    """
    kl_loss = nn.KLDivLoss(reduction='batchmean')
    source_audio_student = F.softmax(logits_source/temperature, dim=1) # [B, 256]
...
def train(av_model, train_loader, test_loader, args, label_map: dict, device):
    ...
    prompt_text = get_prompt_text(label_map) # ['normal','fighting','shooting', 'riot', 'abuse', 'car accident', 'explosion'] # len=7
    ap_best = 0
    epoch = 0
    ...
    for e in range(epoch, args.max_epoch):
        ...
        for i, item in enumerate(train_loader):
            ...
            text_labels = get_batch_label(text_labels, prompt_text, label_map).to(device) # [B, N]
            ...
            '''
            # CoCoOp
            loss3 = torch.zeros(len(text_features)).to(device)
            text_feature_normal = (text_features[:, 0] / text_features[:, 0].norm(dim=-1, keepdim=True)).squeeze(1) # [B, 512]
            for j in range(1, text_features.shape[1]): # N번
                text_feature_abr = (text_features[:, j] / text_features[:, j].norm(dim=-1, keepdim=True)).squeeze(1) # [B, 512]
                loss3 += torch.abs((text_feature_normal * text_feature_abr).sum(dim=-1)) # [B]
            loss3 = loss3.sum(dim=0) / 6 # [1,]
            loss_total3 += loss3.item()
            '''
            ...
            #loss4_1 = distill_cosine_fixed_teacher(v_logits.squeeze(-1), a_logits.squeeze(-1), 3.0) + distill_cosine_fixed_teacher(a_logits.squeeze(-1), v_logits.squeeze(-1), 3.0)
            #loss5_1 = distill_cosine_fixed_teacher(logits_av, v_logits.squeeze(-1), 3.0)
            #loss6_1 = CLAS2(a_logits.squeeze(-1), text_labels, feat_lengths, device) # BCE_Audio

            loss4_1 = DISTILL(v_logits.squeeze(-1), a_logits.squeeze(-1), 3.0) # V2A
            loss4_2 = DISTILL(a_logits.squeeze(-1), v_logits.squeeze(-1), 3.0) # A2V
            loss5 = DISTILL(logits_av, v_logits.squeeze(-1), 3.0) # AV2V
            loss6 = CLAS2(a_logits.squeeze(-1), text_labels, feat_lengths, device) # BCE_Audio
            #loss7 = DISTILL(logits_av, a_logits.squeeze(-1), 3.0) # AV2A
            #loss7 = DISTILL(logits_av, a_logits.squeeze(-1), 3.0) + DISTILL(a_logits.squeeze(-1), logits_av, 3.0) # Chain-av-audio
            #loss8 = DISTILL(v_logits.squeeze(-1), logits_av, 3.0) + DISTILL(logits_av, v_logits.squeeze(-1), 3.0) # Chain av-visual
            #loss9 = CLAS2(v_logits.squeeze(-1), text_labels, feat_lengths, device) # BCE_Visual

            added_loss = loss6 + loss4_1 +loss4_2 + loss5
            ...
            if step % 4800 == 0 and step != 0:
                ...
                #print("  AV Loss1: {:.4f}, AV Loss2: {:.4f}, AV Loss3: {:.4f}, CMAL Loss: {:.4f}, Added Loss: {:.4f}".format(loss_total1/(i+1), loss_total2/(i+1), loss_total3/(i+1), loss_total_cmal/(i+1), added_loss/(i+1)))
                
                # 중간 디버깅 평가: 테스트 데이터셋으로 AUC, AP, mAP 출력
                print("  --> Running mid-epoch evaluation ...")
                auc, ap, mAP = test(av_model, None, test_loader, args.visual_length, prompt_text, gt, gtsegments, gtlabels, device)
                sys.stdout.write(f"      [Mid-Epoch] AUC: {auc:.4f}, AP: {ap:.4f}, mAP: {mAP:.4f} \n")
                sys.stdout.flush()
                """
                if ap == 0.8619469893400715:
                    checkpoint = {
                        'epoch': e,
                    ...
                    torch.save(checkpoint, './No_CMAL/No_CmalXdModel.pth')
                """
...
if __name__ == '__main__':
    ...
    #    av_model = DataParallel(av_model)
    #    av_model = av_model.to(device)

    train(av_model, train_loader, test_loader, args, label_map, device)
