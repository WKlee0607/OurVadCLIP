...
class CLIPVAD(nn.Module):
    ...
    def encode_video(self, images, audio, padding_mask, lengths):
        ...
        #x_audio = x_audio_enhanced.permute(1, 0, 2) # [B, 256, 512]
    ...
    def forward(self, visual, audio, padding_mask, text, lengths):
        ...
        text_features = text_features + self.mlp_t(text_features)
...
class CLIPVAD(nn.Module):
    def __init__(self,
        ...
        self.visual_classifier = nn.Sequential(
            nn.Linear(visual_width, visual_width // 8),
            QuickGELU(),
            nn.Linear(visual_width // 8, 1),
        )
        self.audio_classifier = nn.Sequential(
            nn.Linear(visual_width, visual_width // 8),
            QuickGELU(),
        ...
        self.av_classifier = nn.Sequential(
            nn.Linear(visual_width * 2, visual_width // 4),
            QuickGELU(),
            nn.Linear(visual_width // 4, visual_width // 8),
            QuickGELU(),
            nn.Linear(visual_width // 8, 1),
        )
    ...
    def forward(self, visual, audio, padding_mask, text, lengths):
        visual_features, audio_features = self.encode_video(visual, audio, padding_mask, lengths) # [B, 256, 512]
        
        # Each Branch Logits -> 새 loss 정의 
        v_logits = self.visual_classifier(visual_features)
        a_logits = self.audio_classifier(audio_features)

        # Fusion
        av_features = torch.cat([visual_features, audio_features], dim=-1) # [B, 256, 1024]
        logits_av_3d = self.av_classifier(av_features) # [B, 256, 1]
        logits_av = logits_av_3d.squeeze(-1)
        ...
        # 3차원 logits_av_3d를 사용하여 permute
        logits_attn = logits_av_3d.permute(0, 2, 1)  # [batch_size, 1, seq_len]
        visual_attn = logits_attn @ visual_features # 원본: visual_attn = logits_attn @ visual_features
        visual_attn = visual_attn / visual_attn.norm(dim=-1, keepdim=True)
        visual_attn = visual_attn.expand(visual_attn.shape[0], text_features_ori.shape[0], visual_attn.shape[2])
        text_features = text_features_ori.unsqueeze(0)
        text_features = text_features.expand(visual_attn.shape[0], text_features.shape[1], text_features.shape[2])
        text_features = text_features + visual_attn
        text_features = text_features + self.mlp_t(text_features) # 원본
        
        visual_features_norm = visual_features / visual_features.norm(dim=-1, keepdim=True) #원본: visual_features_norm = visual_features / visual_features.norm(dim=-1, keepdim=True)
        text_features_norm = text_features / text_features.norm(dim=-1, keepdim=True)
        text_features_norm = text_features_norm.permute(0, 2, 1)
        logits2 = visual_features_norm @ text_features_norm.type(visual_features_norm.dtype) / 0.07
        return text_features_ori, logits2, logits_av, v_logits, a_logits
