...
class CLIPVAD(nn.Module):
    def __init__(self,
        ...
        # av_classifier
        self.fc1 = nn.Linear(visual_width*2, visual_width)
        self.fc2 = nn.Linear(visual_width, 128)
    ...
    def forward(self, visual, audio, padding_mask, text, lengths):
        ...
        #logits_av_3d = self.av_classifier(combined_features)  # 3차원 텐서 [batch_size, seq_len, 1]
        # av_classifier
        logits_av_3d = self.gelu(self.fc1(combined_features))
        logits_av_3d = self.drop_out(logits_av_3d)
        logits_av_3d = self.gelu(self.fc2(logits_av_3d))
        logits_av_3d = self.drop_out(logits_av_3d)
        logits_av_3d = self.gelu(self.fc3(logits_av_3d)) # [B, 256, 1]
