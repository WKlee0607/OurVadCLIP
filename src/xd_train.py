...
def train(model, train_loader, test_loader, args, label_map: dict, device, rank):
    ...
    for e in range(epoch, args.max_epoch):
        ...
        #for item in tqdm(train_loader, desc=f"Extracting features in {rank}..."):
            ...
            text_features, logits_coarse, logits_fine, logits_visual, logits_av = model(visual_feat, audio_feat, None, prompt_text, feat_lengths) 
            ...
            loss_all = loss1 + loss2 + loss3 * 1e-4
            ...
            if step % 4800 == 0 and step != 0:
                ...
                print("  Coarse: {:.4f}, Fine Loss: {:.4f}, Loss3: {:.4f}".format(
