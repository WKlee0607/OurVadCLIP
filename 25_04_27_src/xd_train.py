...
def DISTILL(logits_target, logits_source, temperature):
    ...
    - logits_source: [B, 256]
    """
    #logits_source = torch.clamp(logits_source, min=-100.0, max=100.0)
    #logits_target = torch.clamp(logits_target, min=-100.0, max=100.0)
    #logits_source = normalize_logits(logits_source)
    #logits_target = normalize_logits(logits_target)
...
def train(av_model, v_model, train_loader, test_loader, args, label_map: dict, device):
    ...
    for e in range(epoch, args.max_epoch):
        ...
        for i, item in enumerate(train_loader):
            ...
            # 최종 손실 계산 (CMAL 손실 추가)
            loss_av = loss1 + loss2 + loss3 * 1e-4 + added_loss + cmal_loss 
...
if __name__ == '__main__':
    ...
    v_model = v_model.to(torch.float32)
