...
class CLIPVAD(nn.Module):
    def __init__(self,
        ...
        """
        ## --------- aggregation---------
        self.mlp_v = nn.Sequential(OrderedDict([ 
            ("c_fc", nn.Linear(visual_width, visual_width * 4)), 
            ("gelu", QuickGELU()),
            ("c_proj", nn.Linear(visual_width * 4, visual_width))
        ]))
        ...
        self.linear_res = nn.Sequential(
            nn.Linear(visual_width * 2, visual_width * 2),
            QuickGELU(),
            nn.Linear(visual_width * 2, visual_width),
        )
        """
    ...
    def forward(self, visual, audio, padding_mask, text, lengths):
        ...
        # Visual Prompt
        text_features = text_features + visual_attn # [B, N, 512]
        text_features = text_features + self.mlp_t(text_features) # [B, N, 512]
        ...
        combined_features = torch.cat([
            self.mlp_v(logits_visual * visual_features) + visual_features, self.mlp_a(logits_audio * audio_features) + audio_features], 
            dim=-1
        )
        logits_av_3d = self.av_classifier(combined_features)  # 3차원 텐서 [batch_size, seq_len, 1] -> [B, 256, 1]
        logits_av = logits_av_3d.squeeze(-1)  # 2차원 텐서 [batch_size, seq_len] -> [B, 256]
