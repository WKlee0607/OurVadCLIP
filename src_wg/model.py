...
class CrossModalityAttention(nn.Module):
    ...
    def forward(self, x1, x2):
        # x1 attends to x2
        x1_norm = self.ln(x1)
        q = self.query_proj(x1_norm)
        k = self.key_proj(x2)
        v = self.value_proj(x2)
        
        attn_output, _ = self.attn(q, k, v)
        x1 = x1 + attn_output
        x1 = x1 + self.mlp(self.ln(x1))
        return x1



####### ---audio & audio-visual model--------------------------------------------------------------------------------
class Classifier(nn.Module):
    def __init__(self,visual_width: int):
        """
        This is Classifier Moudle. 
        """
        super().__init__()
    ...
    def forward(self, x: torch.Tensor):
        return self.classifier(x + self.mlp(x))

class LGT(nn.Module):
    def __init__(self, visual_length, visual_width, visual_layers, visual_head, attn_window, device):
        """
        This if LGT Module
        """
        super().__init__()
    ...
    def adj4(self, x, seq_len):
        ...
        output = torch.zeros_like(x2, device=self.device)
        if seq_len is None:
    ...
    def forward(self, images, lengths):
        """
        Return visual(audio) features: [B, 256, 512]
        """
...
class CLIPVAD(nn.Module):
    def __init__(self,
        ...
        self.clipmodel, _ = clip.load("ViT-B/16", device) # pre-trained
        ...
        # Fusion Features 
        self.fuse_layer = CrossModalityAttention(visual_width, visual_head)

        # LGT input - pos emb
        self.video_LGT = LGT(visual_length, visual_width, visual_layers, visual_head, attn_window, device)
        self.audio_LGT = LGT(visual_length, visual_width, visual_layers, visual_head, attn_window, device)
        self.fuse_LGT = LGT(visual_length, visual_width, visual_layers, visual_head, attn_window, device)

        # Each Classifier -> Distillation 될 부분(Student)
        self.visual_classifier = Classifier(visual_width)
        self.audio_classifier = Classifier(visual_width)
        self.av_classifier = Classifier(visual_width) #Classifier(visual_width)

        # Visual Prompt
        self.mlp_vp = nn.Sequential(OrderedDict([
            ("c_fc", nn.Linear(visual_width, visual_width * 4)),
            ("gelu", QuickGELU()),
            ("c_proj", nn.Linear(visual_width * 4, visual_width))
        ]))
    ...
    def encode_video(self, images, audio, padding_mask, lengths):
        """
        : Processing LGT Adapter to extract featrues on video & audio
        ...
        ## Process Fusion & LGT
        cond_x_visual = self.fuse_LGT(self.fuse_layer(torch.cat((images, audio), dim=-1)), lengths) #self.fuse_LGT(self.fuse_layer(images, audio), lengths) 
        x_visual = self.video_LGT(images, lengths)
        x_audio = self.audio_LGT(audio, lengths)

        return x_visual, x_audio, cond_x_visual 

    def encode_textprompt(self, text):
        word_tokens = clip.tokenize(text).to(self.device)
        word_embedding = self.clipmodel.encode_token(word_tokens)
        text_embeddings = self.text_prompt_embeddings(torch.arange(77, device=self.device)).unsqueeze(0).repeat(len(text), 1, 1)
        text_tokens = torch.zeros(len(text), 77, device=self.device)
    ...
    def forward(self, visual, audio, padding_mask, text, lengths):
        ## Extract features throught each LGT Adapter
        visual_features, audio_features, fusion_features = self.encode_video(visual, audio, padding_mask, lengths)
        
        # logits
        logits_av = self.av_classifier(fusion_features) # audio-visual logits
        logits_visual = self.visual_classifier(visual_features) # visual logits
        logits_audio = self.audio_classifier(audio_features) # audio logits

        ## A-branch
        text_features_ori = self.encode_textprompt(text)

        ## Visual Prompt(visual_feat & visual_score)
        """
        logits_attn = logits_visual.permute(0, 2, 1) 
        visual_attn = logits_attn @ visual_features # A: aggregation
        ...
        text_features = text_features + self.mlp_vp(text_features)
        
        visual_features_norm = visual_features / visual_features.norm(dim=-1, keepdim=True)
        text_features_norm = text_features / text_features.norm(dim=-1, keepdim=True)
        text_features_norm = text_features_norm.permute(0, 2, 1)
        logits_fine = visual_features_norm @ text_features_norm.type(visual_features_norm.dtype) / 0.07 # fine-grained heatmap
        """

        ## Visual Prompt(cond_feat & cond_score)
        logits_attn = logits_visual.permute(0, 2, 1) 
        visual_attn = logits_attn @ visual_features # A: aggregation
        visual_attn = visual_attn / visual_attn.norm(dim=-1, keepdim=True)
        visual_attn = visual_attn.expand(visual_attn.shape[0], text_features_ori.shape[0], visual_attn.shape[2])
        ...
        #return text_features_ori, logits_visual, logits_fine, logits_visual, logits_audio, logits_av
        return text_features_ori, logits_av, logits_fine, logits_visual, logits_audio, logits_av
        #return text_features_ori, logits_visual, logits_fine, logits_visual, logits_audio, visual_features, audio_features
...
class CLIPVAD(nn.Module):
    def __init__(self,
        ...
        # Audio-visual Classifier -> Distillation 할 부분(Teacher)
        self.av_classifier = Classifier(visual_width) #Classifier(visual_width)
    ...
    def encode_video(self, images, audio, padding_mask, lengths):
        ...
        # LGT - Local Transformer Encoder
        x_visual, _ = self.temporal((images, None)) 
        x_audio, _ = self.audio_temporal((audio, None)) # 수정 -> T 다름
        ...
        adj = self.adj4(x_audio, lengths)
        disadj = self.disAdj_a(x_audio.shape[0], x_audio.shape[1])
        x1_h = self.gelu_a(self.gc1_a(x_audio, adj))
        x2_h = self.gelu_a(self.gc3_a(x_audio, disadj))
        x1 = self.gelu_a(self.gc2_a(x1_h, adj))
        x2 = self.gelu_a(self.gc4_a(x2_h, disadj))
        x_audio = torch.cat((x1, x2), 2)
        x_audio = self.linear_a(x_audio)

        return x_visual, x_audio # features - [B, visual_width]?
    ...
    def forward(self, visual, audio, padding_mask, text, lengths):
        ...
        ## Process Fusion 
        fused_audio = self.cross_attn_v2a(visual_features, audio_features)
        fused_video = self.cross_attn_a2v(audio_features, visual_features)
        combined_feat = fused_audio + fused_video 
        

        ## Process Each logits(Classifier Module) in C-branch
        logits_visual = self.visual_classifier(visual_features) # visual logits - logits_coarse
        #logits_audio = self.audio_classifier(audio_features) # audio logits
        logits_av = self.av_classifier(combined_feat).squeeze(-1) # audio-visual logits
        ...
        #return text_features_ori, logits_visual, logits_fine, logits_visual, logits_audio, logits_av
        return text_features_ori, logits_visual, logits_fine, logits_visual, logits_av
        #return text_features_ori, logits_visual, logits_fine, logits_visual, logits_audio, visual_features, audio_features
...
class CLIPVAD(nn.Module):
    ...
    def forward(self, visual, audio, padding_mask, text, lengths):
        ...
        logits1 = torch.maximum(logits_visual, logits_audio) # concat from visual-audio
        ...
        text_features_norm = text_features / text_features.norm(dim=-1, keepdim=True)
        text_features_norm = text_features_norm.permute(0, 2, 1)
        logits2 = visual_features_norm @ text_features_norm.type(visual_features_norm.dtype) / 0.07 # fine-grained
